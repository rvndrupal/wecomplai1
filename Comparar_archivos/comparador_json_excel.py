...
NUMERO_FILAS_VALIDAR = 100  # Cambiar a None para procesar TODO el archivo
...
def cargar_excel(ruta, num_filas=None):
    """
    Carga el archivo Excel base
    ...
    if num_filas:
        df = pd.read_excel(ruta, nrows=num_filas)
        print(f"   ✅ Cargadas {len(df)} filas (limitado a {num_filas})")
    else:
        df = pd.read_excel(ruta)
        print(f"   ✅ Cargadas {len(df)} filas (archivo completo)")
...
def aplicar_formato_excel(ruta_excel, nombre_hoja, es_detallado=False):
    """
    Aplica formato condicional y estilos al Excel
    ...
    # Aplicar formato condicional si es reporte detallado
    if es_detallado:
...
def generar_reportes(resultados_detallados, resumen_filas):
    ...
    info_data = {
        ...
        'Valor': [
            ...
            JSON_PATH,
            len(resumen_filas),
            f'VALIDACIÓN ({NUMERO_FILAS_VALIDAR} filas)' if NUMERO_FILAS_VALIDAR else 'COMPLETO',
            'RVN - Cursos QA Automatizados'
        ]
    }
...
def main():
    ...
    """
    print("\n" + "=" * 80)
    print("🚀 COMPARADOR JSON vs EXCEL - RVN Cursos QA Automatizados")
    print("=" * 80)
    ...
    if NUMERO_FILAS_VALIDAR:
        ...
        print(f"⚙️  Modo: COMPLETO - Procesando todo el archivo")
    
    print("=" * 80)
    
    # 1. Cargar archivos
    df_excel = cargar_excel(EXCEL_BASE_PATH, NUMERO_FILAS_VALIDAR)
